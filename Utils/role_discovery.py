...
def role_discovery(logfile, resource_attr, sim_percentage):
    # Get all unique tasks
    data = logfile.get_data()
    tasks = list(set(list(data[logfile.activity])))
    try:
        tasks.remove('Start')
    except Exception:
        pass
    tasks = [dict(index=i,data=tasks[i]) for i in range(0,len(tasks))]
    users = list(set(list(data[resource_attr])))
    try:
        users.remove('Start')
    except Exception:
        pass
    ...
    print(tasks)
    print(users)
    np_data = data[[logfile.activity, resource_attr]].values
    data_transform = list(map(lambda x: [find_index(tasks, x[0]),find_index(users, x[1])], np_data ))
...
if __name__ == "__main__":
    ...
    logfile = LogFile("../Data/BPIC12.csv", ",", 0, 10000, "completeTime", "case", activity_attr="task")
    print(role_discovery(logfile, "user", 0.5))
